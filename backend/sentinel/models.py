...
class ProjectTagMapping(models.Model):
    ...
    class Meta:  # 元数据类
        ...
        db_table = "projecttag_mapping"  # 指定数据库表名，与数据库一致
...
class BlogTagMapping(models.Model):
    ...
    class Meta:  # 元数据类
        ...
        db_table = "BlogTag_Mapping"  # 指定数据库表名，与数据库一致
...
class SystemSettings(models.Model):
    ...
    def __str__(self):  # 字符串表示方法
        return f"{self.setting_key}: {self.setting_value}"  # 返回设置键和设置值的组合作为对象的字符串表示


class EmailSubscriber(models.Model):
    """邮件订阅者模型"""
    email = models.EmailField("邮箱地址", max_length=100, unique=True)  # 邮箱地址字段，唯一
    name = models.CharField("姓名", max_length=50, null=True, blank=True)  # 姓名字段，可为空
    subscribed_at = models.DateTimeField("订阅时间", auto_now_add=True)  # 订阅时间字段，自动添加当前时间
    is_active = models.BooleanField("是否活跃", default=True)  # 是否活跃字段，默认为True
    confirmation_token = models.CharField("确认令牌", max_length=100, null=True, blank=True)  # 确认令牌字段，可为空
    confirmed = models.BooleanField("是否已确认", default=False)  # 是否已确认字段，默认为False
    last_sent_at = models.DateTimeField("最后发送时间", null=True, blank=True)  # 最后发送时间字段，可为空
    unsubscribe_token = models.CharField("退订令牌", max_length=100, null=True, blank=True)  # 退订令牌字段，可为空

    class Meta:  # 元数据类
        verbose_name = "邮件订阅者"  # 单数形式的显示名称
        verbose_name_plural = "邮件订阅者"  # 复数形式的显示名称
        db_table = "EmailSubscribers"  # 指定数据库表名
        indexes = [  # 创建索引
            models.Index(fields=["email"]),  # 为邮箱地址字段创建索引
            models.Index(fields=["is_active"]),  # 为是否活跃字段创建索引
        ]

    def __str__(self):  # 字符串表示方法
        return self.email  # 返回邮箱地址作为对象的字符串表示
